...
county_demo1_binned <- county_demo1 %>%
  mutate(
    Inc_Bin = case_when(
      Inc >= 50000 ~ "50,000 or more",
      Inc >= 40000 ~ "40,000 to 49,999",
      Inc >= 35000 ~ "35,000 to 39,999",
      Inc >= 30000 ~ "30,000 to 34,999",
      TRUE ~ "Less than 30,000"
    ),
    Inc_Bin = factor(Inc_Bin, levels = c("50,000 or more", "40,000 to 49,999", "35,000 to 39,999", "30,000 to 34,999", "Less than 30,000"))
  )
...
p_main <- ggplot() +
  ...
  scale_fill_manual(
    ...
    # ),
    values = c(
      "50,000 or more" = "#027967",
      "40,000 to 49,999" = "#009987",
      "35,000 to 39,999" = "#00B3A5",
      "30,000 to 34,999" = "#81D4CA",
      "Less than 30,000" = "#E8F4F8"
    ),
    na.value = "grey90"
  ) +
  labs(title = "Per Capita Income by County",
       subtitle = "U.S. median income: $28,392") +
  theme_minimal() +
  theme(
